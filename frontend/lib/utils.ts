...
/**
 * Build the token source for a LiveKit session.
 *
 * Works for both the local `/api/token` route and a LiveKit Cloud Sandbox
 * connection-details endpoint, dispatching the agent by name.
 *
 * @param appConfig - The app configuration
 * @returns A configurable token source
 */
export function getConnectionTokenSource(appConfig: AppConfig) {
  const usingSandbox = typeof process.env.NEXT_PUBLIC_CONN_DETAILS_ENDPOINT === 'string';

  return TokenSource.custom(async () => {
    const endpoint = usingSandbox
