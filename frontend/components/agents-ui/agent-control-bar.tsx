...
import { AgentPhotoCaptureButton } from '@/components/agents-ui/agent-photo-capture-button';
...
export interface AgentControlBarControls {
  ...
  chat?: boolean;
  /**
   * Whether to show the in-call photo-capture button.
   *
   * @defaultValue true (if data publish permission is granted)
   */
...
export interface AgentControlBarProps extends UseInputControlsProps {
  ...
  /** The callback for when a photo is captured and sent to the agent. */
...
export function AgentControlBar({
  ...
  const visibleControls = {
    ...
    photoCapture: controls?.photoCapture ?? publishPermissions.data,
  };
    ...
    >
      ...
      <div className="flex gap-1">
        <div className="flex grow gap-1">
          ...
          {/* Take a photo */}
          {visibleControls.photoCapture && (
            <AgentPhotoCaptureButton
              variant={variant}
              onPhotoSent={(file) => {
                toast.info('Photo sent — one moment while I take a look…');
                onPhotoSent?.(file);
              }}
              onError={(error) => {
                console.error('failed to send photo to agent', error);
                toast.error("Couldn't send that photo. Please try again.");
              }}
            />
          )}
