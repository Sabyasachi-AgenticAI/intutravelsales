...
// Image metadata
export const alt = 'intuService — AI Voice Service Advisor';
export const size = {
  width: 1200,
  height: 628,
};
...
export default async function Image() {
  ...
  // intuService logo mark (used top-left and center)
  const { base64: logoSrcBase64, dimensions: logoDimensions } = await getImageData(
    'public/intuservice-logo.svg'
  );
  const markSize = scaleImageSize(logoDimensions, 40);
  const centerSize = scaleImageSize(logoDimensions, 72);
    ...
    (
      ...
      >
        ...
        >
          ...
          <img src={logoSrcBase64} width={markSize.width} height={markSize.height} />
          <div style={{ fontSize: 28, fontWeight: 700, fontFamily: 'Everett', color: 'white' }}>
            intuService
          </div>
        </div>
        {/* logo */}
        ...
        >
          {/* eslint-disable-next-line jsx-a11y/alt-text */}
          <img src={logoSrcBase64} width={centerSize.width} height={centerSize.height} />
        </div>
        {/* title */}
        <div
          style={{
